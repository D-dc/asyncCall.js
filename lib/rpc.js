...
var RPC = function(socket, options) {
    ...
    this.lease = new Lease(Infinity); // start with dummy lease
};

RPC.prototype.newLease = function(lease){
    if(this.lease)
        this.lease.revoke();
...
RPC.prototype._exposeFunction = function(name, func) {
    ...
    if (this.exposedFunctions[name])
        throw new Exception("No function overloading, overwriting previous ", name);
